...
public abstract class BaseRecylerViewAdapter<VH extends RecyclerView.ViewHolder, DI extends AndroidModel>
    ...
    public void setWithData(List<Record> data1){
        this.data1 = data1;
        notifyDataSetChanged();
    }

    public void addWithData(List<Record> data){
        int initialCount = this.data1.size();
        int newCount = 0;
        for(Record defaultItem : data){
            if(isDataWith(defaultItem)){
                this.data1.add(defaultItem);
                newCount++;
            }
        }
    ...
    private boolean isDataWith(Record item){
        for(Record defaultItem : this.data1){
